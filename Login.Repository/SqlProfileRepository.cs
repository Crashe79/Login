...
using System.Text;
using Login.Repository.DataProvider;
...
namespace Login.Repository
{
    ...
    {
        private readonly string _conn = ConfigurationManager.ConnectionStrings["MySqlWrioProfile"].ConnectionString;

        public IUserAccount GetUserBy(string id)
        {
            UserAccount user;
        ...
        public IUserAccount GetUsersBy(string email)
        {
            UserAccount user;
            using (var context = new ProfileDbManager(_conn))
            {
        ...
        public IUserAccount GetUserId(string userIdentity)
        {
        ...
        public void UpdateUser(string salt, string hash, IUserAccount curUser)
        {
            var entity = new UserAccount();
        ...
        public void SaveUser(IUserAccount user)
        {
            try
            {
                using (var db = new ProfileDbManager(_conn))
        ...
        public IUserAccount SaveUser(DateTime date, string salt, string hash, IRegistrationModel model,
            string facebookId = null, string googleId = null, string twitterId = null)
        {
        ...
        {
            string query = string.Format("DELETE FROM UserAccount WHERE Id= '{0}'", id);
            using (var db = new ProfileDbManager(_conn))
            {
        ...
        {
            IUserAccount user = GetUsersBy(email);
            if (user == null)
            {
                return false;
            }
        ...
        public IUserAccount CreateEmptyUserAccount()
        {
